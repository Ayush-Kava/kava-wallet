...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const sourcePublicId = source_id != null ? parsePublicId(source_id) : undefined;

    if (source_id != null && !sourcePublicId) {
      return errorResponse(ERRORS.GENERIC_BAD_REQUEST);
    }

    const updated = await updateFunding(user.id, publicId, {
      source_type,
      source_id: sourcePublicId ?? undefined,
      allocated_amount,
    });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const user = await authUser();
    const deleted = await removeFunding(user.id, publicId);
